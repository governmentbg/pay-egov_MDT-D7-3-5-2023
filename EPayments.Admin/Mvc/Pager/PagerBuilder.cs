...
namespace EPayments.Admin.Mvc.Pager
{
    internal class PagerBuilder
    {
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            if (!String.IsNullOrWhiteSpace(_pagerOptions.ContainerTagClass))
            {
                tb.AddCssClass(_pagerOptions.ContainerTagClass);
            }
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<li><a href=\"#\" onclick=\"return false;\">{0}</a></li>", item.Text));
            return CreateWrappedPagerElement(item, String.Format("<li{0}><a href='{1}'>{2}</a></li>", String.IsNullOrWhiteSpace(url) ? " class=\"active\" onclick=\"return false;\"" : String.Empty, String.IsNullOrWhiteSpace(url) ? "#" : url, item.Text));
        }
        ...
        private RouteValueDictionary GetCurrentRouteValues(ViewContext viewContext)
        {
            ...
            if (_pagerOptions.AddQueryStringValuesMissingInRouteValues)
            {
                if (rq != null && rq.Count > 0)
                {
                    List<string> invalidParamsList = new List<string>();
                    invalidParamsList.Add("x-requested-with");
                    invalidParamsList.Add("xmlhttprequest");
                    invalidParamsList.Add(_pagerOptions.PageIndexParameterName.ToLower());
                    //Add routeValue parameters - it is need because otherwise the routeValue parameters are overwritten by queryString params
                    foreach (var rv in routeValues)
                    {
                        invalidParamsList.Add(rv.Key.ToLower());
                    }
                    ...
                    foreach (string key in rq.Keys)
                    {
                        ...
                        if (!string.IsNullOrEmpty(key) && !invalidParamsList.Any(e => e == key.ToLower()))
                        {
