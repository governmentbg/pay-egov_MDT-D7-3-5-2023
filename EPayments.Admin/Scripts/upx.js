document.addEventListener('DOMContentLoaded', function() {
    ...
    if ($('[data-toggle=confirmation-post]').length != 0) {
        $('[data-toggle=confirmation-post]').confirmation({
            container: 'body',
            btnOkLabel: 'Да',
            btnCancelLabel: 'Не',
            popout: true,
	...
	if ($('.aisclients').length != 0) {
		$('.aisclients').click(function () {
			if ($(this).hasClass('opened')) {
	...
	//payment-toggle in distribution payments
	if ($('.payment-toggle').length != 0) {
